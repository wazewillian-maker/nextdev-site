    ...
    <section className="section portfolio" id="portfolio">
      ...
      <div className="project-grid">{projects.map((project, index) => (
        <article className={`project-card reveal${project.featured ? ' project-featured' : ''}`} key={project.id}>
          ...
          <div className="project-info"><div><small>{project.category}</small><h3>{project.title}</h3><p>{project.description}</p>{project.link && <a className="project-link" href={project.link} target="_blank" rel="noopener noreferrer" aria-label={`Ver projeto ${project.title} em uma nova aba`}>Ver projeto <span>↗</span></a>}</div><span aria-hidden="true">↗</span></div>
        </article>
      ))}</div>
    </section>
  )
